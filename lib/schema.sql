...
-- ============================================================================
-- 7b. TRANSFERENCIA ENTRE ESTOQUES  (documento: origem -> destino)
-- O estoque e separado por local (tabela `estoque`: variacao x loja/deposito) e
-- a conversa entre eles acontece por transferencia, com documento numerado,
-- historico nas duas pontas e possibilidade de cancelar (estorno).
-- ============================================================================

CREATE TABLE IF NOT EXISTS transferencias (
  id                  INTEGER PRIMARY KEY AUTOINCREMENT,
  numero              TEXT UNIQUE,
  loja_origem         INTEGER NOT NULL REFERENCES lojas(id),
  loja_destino        INTEGER NOT NULL REFERENCES lojas(id),
  ...
  criado_em           TEXT NOT NULL DEFAULT (datetime('now','localtime')),
  CHECK (loja_origem <> loja_destino)
);

CREATE INDEX IF NOT EXISTS ix_transf_data    ON transferencias(data);
CREATE INDEX IF NOT EXISTS ix_transf_origem  ON transferencias(loja_origem);
CREATE INDEX IF NOT EXISTS ix_transf_destino ON transferencias(loja_destino);
...
CREATE INDEX IF NOT EXISTS ix_transfit_transf ON transferencias_itens(transferencia_id);
CREATE INDEX IF NOT EXISTS ix_transfit_var    ON transferencias_itens(variacao_id);
...
-- Estoque CONSOLIDADO (soma de todos os locais). O JOIN e agregado de proposito:
-- sem ele cada SKU apareceria uma vez por local e todo total do sistema dobraria.
-- O saldo de cada local fica em vw_estoque_loja.
LEFT JOIN (SELECT variacao_id,
                  SUM(quantidade)                AS quantidade,
                  SUM(reservado)                 AS reservado,
                  SUM(quantidade) - SUM(reservado) AS disponivel
             FROM estoque GROUP BY variacao_id) est ON est.variacao_id = v.id;

-- Cadastro dos estoques (locais) com o resumo de cada um
DROP VIEW IF EXISTS vw_estoques;
CREATE VIEW vw_estoques AS
SELECT
  l.id                    AS loja_id,
  l.nome,
  l.apelido,
  ...
  l.cidade,
  l.uf,
  CASE WHEN l.eh_deposito = 1 THEN 'deposito' ELSE 'loja' END AS tipo,
  (SELECT COUNT(DISTINCT e.variacao_id) FROM estoque e
    WHERE e.loja_id = l.id AND e.quantidade > 0)               AS skus,
  (SELECT COALESCE(SUM(e.quantidade),0) FROM estoque e
    WHERE e.loja_id = l.id)                                    AS pecas,
  (SELECT COALESCE(SUM(e.quantidade * v.custo_medio),0)
     FROM estoque e JOIN variacoes v ON v.id = e.variacao_id
    WHERE e.loja_id = l.id)                                    AS valor_custo,
  (SELECT COALESCE(SUM(e.quantidade * v.preco_venda),0)
     FROM estoque e JOIN variacoes v ON v.id = e.variacao_id
    WHERE e.loja_id = l.id)                                    AS valor_venda,
  (SELECT COUNT(*) FROM transferencias t
    WHERE t.loja_origem = l.id OR t.loja_destino = l.id)       AS transferencias
FROM lojas l;

-- Posicao de estoque POR LOCAL (variacao x loja/deposito): a base das telas de
-- estoque separado, da transferencia e da venda no PDV.
DROP VIEW IF EXISTS vw_estoque_loja;
CREATE VIEW vw_estoque_loja AS
SELECT
  e.variacao_id,
  e.loja_id,
  l.nome                      AS loja,
  ...
  l.eh_deposito,
  l.padrao                    AS loja_padrao,
  CASE WHEN l.eh_deposito = 1 THEN 'deposito' ELSE 'loja' END AS loja_tipo,
  vv.sku,
  vv.ean,
  vv.codigo_interno,
  vv.produto_id,
  ...
  vv.cor_hex,
  vv.comprimento,
  vv.comprimento_unidade,
  vv.custo_medio,
  vv.preco_venda,
  vv.preco_promocional,
  vv.margem_percentual,
  vv.estoque_min,
  vv.estoque_max,
  vv.ponto_reposicao,
  vv.unidade_estoque,
  ...
  e.disponivel,
  e.atualizado_em,
  (e.quantidade * vv.custo_medio) AS valor_custo,
  (e.quantidade * vv.preco_venda) AS valor_venda,
  CASE
    WHEN e.disponivel <= 0 THEN 'sem_estoque'
    WHEN vv.estoque_min > 0 AND e.disponivel <= vv.estoque_min THEN 'critico'
    WHEN vv.ponto_reposicao > 0 AND e.disponivel <= vv.ponto_reposicao THEN 'repor'
    WHEN vv.estoque_max > 0 AND e.disponivel >= vv.estoque_max THEN 'excesso'
    ELSE 'ok'
  END                         AS situacao_estoque
FROM estoque e
JOIN vw_variacoes vv   ON vv.variacao_id = e.variacao_id
JOIN lojas l           ON l.id = e.loja_id;

-- Historico das transferencias (documento com origem, destino e autor)
DROP VIEW IF EXISTS vw_transferencias;
CREATE VIEW vw_transferencias AS
SELECT
  t.id, t.numero, t.data, t.status, t.itens, t.pecas, t.valor_custo,
  t.loja_origem, t.loja_destino, t.observacoes, t.criado_em,
  t.usuario_id, t.usuario_nome, t.cancelada_em, t.motivo_cancelamento,
  lo.nome                AS origem,
  lo.eh_deposito         AS origem_deposito,
  ld.nome                AS destino,
  ld.eh_deposito         AS destino_deposito,
  COALESCE(t.usuario_nome, u.nome) AS autor
