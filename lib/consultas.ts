...
export async function resumoPeriodo(dias = 30) {
  return (
    await one<{ vendas: number; receita: number; custo: number; lucro: number; pecas: number }>(
      `SELECT COUNT(*) vendas,
              ...
              COALESCE((SELECT SUM(vi.quantidade) FROM vendas_itens vi JOIN vendas v2 ON v2.id = vi.venda_id
                        WHERE v2.status='concluida' AND CAST(v2.data AS DATE) >= ((CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '${dias} days')::date),0) pecas
       FROM vendas
       WHERE status = 'concluida' AND CAST(data AS DATE) >= ((CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '${dias} days')::date`
...
export async function resumoHoje() {
  return (
    await one<{ vendas: number; receita: number; lucro: number }>(
      `SELECT COUNT(*) vendas, COALESCE(SUM(total),0) receita, COALESCE(SUM(total - custo_total),0) lucro
       FROM vendas WHERE status='concluida' AND CAST(data AS DATE) = (CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date`
...
export async function resumoEstoque() {
  return (
    await one<{ skus: number; pecas: number; valor_custo: number; valor_venda: number; sem_estoque: number; criticos: number; repor: number }>(
      `SELECT COUNT(*) skus,
...
export async function vendasPorDia(dias = 30) {
  const linhas = await all<{ dia: string; total: number; qtd: number }>(
    `SELECT CAST(CAST(data AS DATE) AS TEXT) dia, COALESCE(SUM(total),0) total, COUNT(*) qtd
     FROM vendas WHERE status='concluida' AND CAST(data AS DATE) >= ((CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '${dias - 1} days')::date
     GROUP BY CAST(data AS DATE) ORDER BY dia`
  );
  ...
  const inicioHoje = Date.parse(`${hojeBrasil}T00:00:00Z`);
  const saida: { rotulo: string; valor: number; qtd: number; dia: string }[] = [];
  for (let i = dias - 1; i >= 0; i--) {
    const d = new Date(inicioHoje - i * 86400000);
...
export async function topProdutos(dias = 30, limite = 10) {
  return await all<{ produto: string; skus: number; pecas: number; receita: number; lucro: number; margem: number }>(
    `SELECT vv.produto,
     ...
     GROUP BY vv.produto_id, vv.produto
...
export async function topVariacoes(dias = 30, limite = 15) {
  return await all<any>(
    `SELECT vv.sku, vv.produto, vv.cor_codigo, vv.cor, vv.comprimento, vv.comprimento_unidade,
     ...
     WHERE v.status='concluida' AND CAST(v.data AS DATE) >= ((CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '${dias} days')::date
     GROUP BY vi.variacao_id, vv.sku, vv.produto, vv.cor_codigo, vv.cor, vv.comprimento, vv.comprimento_unidade, vv.estoque, vv.disponivel, vv.situacao_estoque
     ORDER BY receita DESC LIMIT ?`,
    limite
  );
}

export async function corQueMaisGira(dias = 90, limite = 8) {
  return await all<any>(
    `SELECT vv.cor, vv.cor_codigo, vv.cor_hex, vv.cor_familia,
     ...
     GROUP BY vv.cor_id, vv.cor, vv.cor_codigo, vv.cor_hex, vv.cor_familia
     ORDER BY pecas DESC LIMIT ?`,
    limite
  );
}

export async function investimentoPorLinha() {
  return await all<any>(
...
export async function curvaABC(dias = 90) {
  const base = await all<{ variacao_id: number; sku: string; produto: string; cor_codigo: string | null; receita: number }>(
    `SELECT vi.variacao_id, vv.sku, vv.produto, vv.cor_codigo, SUM(vi.total) receita
     FROM vendas_itens vi
     JOIN vendas v ON v.id = vi.venda_id
     JOIN vw_variacoes vv ON vv.variacao_id = vi.variacao_id
     WHERE v.status='concluida' AND CAST(v.data AS DATE) >= ((CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '${dias} days')::date
     GROUP BY vi.variacao_id, vv.sku, vv.produto, vv.cor_codigo
     ORDER BY receita DESC`
  );
...
export async function alertasEstoque(limite = 12) {
  return await all<any>(
...
export async function ultimasVendas(limite = 10) {
  return await all<any>(
...
export async function fiadoAberto() {
  return await all<any>(
    `SELECT c.id, c.nome, c.telefone, c.limite_credito,
     ...
     GROUP BY c.id
     HAVING COALESCE(SUM(CASE WHEN f.tipo='compra' THEN f.valor ELSE -f.valor END),0) > 0.01
     ORDER BY saldo DESC`
  );
}

/** Comparativo de custo do mesmo SKU entre fornecedores (quem vende mais barato) */
export async function fornecedoresComparativo(variacaoId: number) {
  return await all<any>(
...
export async function caixaAbertoResumo() {
  const cx = await one<any>(
...
export async function opcoesSimples(tabela: string, ordem = "nome") {
  return await all<{ id: number; nome: string }>(`SELECT id, nome FROM ${tabela} WHERE ativo = 1 ORDER BY ${ordem}`);
}

export async function listaFiltros() {
  const [marcas, categoriasPai, subcategorias, cores, texturas, comprimentos, tiposProduto, materiais, fibras, tecnicas, publicos, linhas, fornecedores, unidades] = await Promise.all([
    opcoesSimples("marcas"),
    all<{ id: number; nome: string }>("SELECT id, nome FROM categorias WHERE pai_id IS NULL ORDER BY nome"),
    all<{ id: number; nome: string; pai_id: number }>("SELECT id, nome, pai_id FROM categorias WHERE pai_id IS NOT NULL ORDER BY nome"),
    all<{ id: number; nome: string; codigo: string; hex: string; familia: string }>(
      "SELECT id, nome, codigo, hex, familia FROM cores WHERE ativo=1 ORDER BY ordem, nome"
    ),
    opcoesSimples("texturas"),
    all<{ id: number; valor: number; unidade: string; rotulo: string }>(
      "SELECT id, valor, unidade, COALESCE(rotulo, valor || ' ' || unidade) rotulo FROM comprimentos WHERE ativo=1 ORDER BY ordem, valor"
    ),
    opcoesSimples("tipos_produto"),
    opcoesSimples("materiais"),
    opcoesSimples("tipos_fibra"),
    opcoesSimples("tecnicas"),
    opcoesSimples("publicos"),
    opcoesSimples("linhas_colecao"),
    all<{ id: number; nome: string; razao_social: string }>(
      "SELECT id, nome_fantasia nome, razao_social FROM fornecedores WHERE ativo=1 ORDER BY nome_fantasia"
    ),
    all<{ sigla: string; nome: string }>("SELECT sigla, nome FROM unidades_medida ORDER BY sigla"),
  ]);
  return { marcas, categoriasPai, subcategorias, cores, texturas, comprimentos, tiposProduto, materiais, fibras, tecnicas, publicos, linhas, fornecedores, unidades };
}
